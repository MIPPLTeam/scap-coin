...
#if defined(NDEBUG)
#error "SafeCapital cannot be compiled without assertions."
#endif
...
CFeeRate minRelayTxFee = CFeeRate(5000000);
...
const string strMessageMagic = "SafeCapital Signed Message:\n";
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX, bool isMasternode)
{
...

    {
        ...
        // Check against previous transactions
        // This is done last to help prevent CPU exhaustion denial-of-service attacks.
        if (!CheckInputs(tx, state, view, false, STANDARD_SCRIPT_VERIFY_FLAGS, true, NULL, isMasternode)) {
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
        nSubsidy = (int)(Params().MaxMoneyOut() / (10.2 * Params().LAST_POW_BLOCK() * COIN)) * COIN;
    } else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 129600) {
         int nDiv = div(nHeight - 1,(int)(60 * 60 * 24 / Params().TargetSpacing()) * 7.5).quot;
         nSubsidy = (0.2 + (double)nDiv / 10) * COIN;
    } else if (nHeight > 129600 && nHeight <= 1576800) {
         int nDiv = div(nHeight - 1,(int)(60 * 60 * 24 / Params().TargetSpacing()) * 60).quot;
         nSubsidy = (1.2 + (double)nDiv / 10) * COIN;
         if (div(nHeight - 1,1440 * 30 + 19).rem == 0) 
             nSubsidy = 200.0 * COIN;
    } else {
        nSubsidy = 3 * COIN;
    }
	
	// yearly decline of production by 40% per year until reached max coin ~10M.
    for (int i = 3 * Params().SubsidyHalvingInterval(); i <= nHeight; i += Params().SubsidyHalvingInterval()) {
        nSubsidy -= nSubsidy * 0.4;
	}	
...
{
    RenameThread("safecapital-scriptch");
...
bool static DisconnectTip(CValidationState& state)
{
    ...
    mempool.removeCoinbaseCollateralSpends(pcoinsTip, pindexDelete->nHeight);
    mempool.check(pcoinsTip);
