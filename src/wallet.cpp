 ...
 */
CFeeRate CWallet::minTxFee = CFeeRate(5000000);
...
CAmount CWallet::GetImmatureCollateral() const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        if (out.tx->vout[out.i].nValue == Params().MasternodeCollateral() && Params().nonCollateralMaturity(out.tx->GetDepthInMainChain(false))
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    if (pcoin->vout[i].nValue == Params().MasternodeCollateral()
                        && Params().nonCollateralMaturity(nDepth)
                        && chainActive.Height() > Params().CollateralMaturityEnforcementHeight())
                    {
                        //LogPrintf("AvailableCoins collateral: skip %d / %d / %d \n", nDepth, pcoin->vout[i].nValue, chainActive.Height());
...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount) const
{
    ...
    if (IsSporkActive(SPORK_17_STAKE_MINIMUM_AGE)) {
...
bool CWallet::MintableCoins()
{
    ...
    if (nBalance <= nReserveBalance || nBalance <= nMinAmount)
        return false;
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 SCAP
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, privateSendDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 SCAP over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputPrivatesendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nPrivatesendRoundsMin, int nPrivatesendRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 SCAP+1 ( bit on if present )
            // bit 1 - 1000 SCAP+1
            // bit 2 - 100 SCAP+1
            // bit 3 - 10 SCAP+1
            // bit 4 - 1 SCAP+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SCAP.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Privatesend non-denominated funds for this transaction that are not equal 10000 SCAP.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-safecapital-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, GetBoolArg("-printcoinstake", false))) {
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setStakeCoins)
    {
        ...
        if (txNew.vout.size() == 2 && (pcoin.first->vout[pcoin.second].scriptPubKey == scriptPubKeyKernel || pcoin.first->vout[pcoin.second].scriptPubKey == aggregateScriptPubKeyHashKernel) && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
    ...
    if (nCredit >= GetStakeSplitThreshold || nCredit >= nStakeSplitThreshold * COIN)
    {
