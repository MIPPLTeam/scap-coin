...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SafeCapitalMiner : generated block is stale");
    }
...
{
    LogPrintf("SafeCapitalMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("safecapital-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        unique_ptr<CBlockTemplate> pblocktemplate(fProofOfStake ? CreateNewBlock(CScript(), pwallet, fProofOfStake) : CreateNewBlockWithKey(reservekey, pwallet));
        if (!pblocktemplate.get())
            continue;
        ...
        LogPrintf("Running SafeCapitalMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
